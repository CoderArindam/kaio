...
-- Add reporter_id to tasks, update canonical view, and fix authorization

ALTER TABLE tasks ADD COLUMN IF NOT EXISTS reporter_id INTEGER REFERENCES users(id) ON DELETE SET NULL;

-- Backfill reporter_id with created_by
UPDATE tasks SET reporter_id = created_by WHERE reporter_id IS NULL AND created_by IS NOT NULL;
...
SELECT
    ...
    ru.avatar_url AS reporter_avatar_url,
...
-- Re-create fn_log_task_time because we CASCADE dropped the view
...
-- Update can_edit_task to include reporter_id
...
BEGIN
    SELECT board_id, assigned_to, reporter_id INTO v_board_id, v_assigned_to, v_reporter_id
    FROM tasks
    WHERE id = p_task_id AND deleted_at IS NULL;
    ...
    -- The assigned user or reporter can edit their task
    IF v_assigned_to = p_user_id OR v_reporter_id = p_user_id THEN
        RETURN TRUE;
    END IF;
