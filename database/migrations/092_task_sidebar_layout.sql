...
-- Add task_sidebar_layout to user_preferences

ALTER TABLE user_preferences ADD COLUMN IF NOT EXISTS task_sidebar_layout JSONB DEFAULT '{"pinned": [], "unpinned": ["assignee", "reporter", "priority", "due_date", "time_tracking", "labels", "created_by"]}'::jsonb NOT NULL;
...
DROP FUNCTION IF EXISTS fn_update_user_preferences(integer, theme_enum, character varying, character varying, boolean, character varying, character varying, character varying, character varying, integer, character varying, character varying, boolean, character varying, boolean, jsonb);
DROP FUNCTION IF EXISTS fn_update_user_preferences(integer, theme_enum, character varying, character varying, boolean, character varying, character varying, character varying, character varying, integer, character varying, character varying, boolean, character varying, boolean);
CREATE OR REPLACE FUNCTION fn_update_user_preferences(
    ...
    p_task_sidebar_layout JSONB DEFAULT NULL
)
RETURNS TABLE (
    id INTEGER,
    user_id INTEGER,
...
BEGIN
    ...
    SET
        ...
        task_sidebar_layout = COALESCE(p_task_sidebar_layout, user_preferences.task_sidebar_layout)
