    ...
    async def get_board_tasks(self, board_id: int, assigned_to: Optional[int], current_user: dict) -> BoardDataResponse:
        try:
            ...
            columns_rows = await self.conn.fetch(
                "SELECT id, name, position, column_type, color, (column_type = 'DONE') AS is_completed FROM board_columns WHERE board_id = $1 AND deleted_at IS NULL ORDER BY position",
                board_id
            )
