...
| **04** | [Database Architecture](./04_DATABASE_ARCHITECTURE.md) | PostgreSQL schema across **48 SQL migration files** (versions 001–047), canonical views (`v_*_canonical`), stored functions (`fn_*`), ERD with all entities (including Timesheets & Policies), RLS/authz functions, session tables, dashboard views, invitation revocation function. |
