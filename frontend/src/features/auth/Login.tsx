...
import { LayoutGrid, Eye, EyeOff, Loader2, KeyRound, ArrowLeft } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const { login, completeOtpLogin } = useAuthStore();
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.otp_required && res.mfa_token) {
        setMfaToken(res.mfa_token);
        setMaskedEmail(res.email || email);
        setStep('OTP');
        toast.success("2FA verification code sent to your email!");
      } else {
        toast.success("Logged in successfully");
        navigate("/dashboard");
      }
    } catch (error: any) {
      const msg = error.response?.data?.detail || (error instanceof Error ? error.message : "Invalid email or password");
      setErrorMessage(msg);
      toast.error(msg);
    } finally {
  ...
  const handleVerifyOtp = async (code: string) => {
    ...
    setOtpError(null);
    try {
      await completeOtpLogin(mfaToken, code);
      toast.success("Authenticated successfully!");
      navigate("/dashboard");
    } catch (error: any) {
      const detail = error.response?.data?.detail || error.message || "Invalid 2FA code";
      setOtpError(detail);
      toast.error(detail);
    } finally {
      setIsSubmitting(false);
  ...
  const handleResendCode = async () => {
    try {
      ...
      if (res.new_mfa_token) {
        setMfaToken(res.new_mfa_token);
      }
          ...
          <>
            ...
            <div className="flex flex-col items-center mb-6 text-center">
              ...
              <p className="text-sm text-brand-text-muted max-w-[300px]">
                Enter the 6-digit security code sent to <span className="font-semibold text-brand-text">{maskedEmail}</span>
              </p>
            </div>
