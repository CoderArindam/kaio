...
export const PasswordSection: React.FC<PasswordSectionProps> = ({ policy, onPasswordChanged }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!meetsPolicy()) { toast.error('Password does not meet the security policy'); return; }
    try {
      setIsSaving(true);
  ...
  const canSubmit = currentPassword && newPassword && confirmPassword && meetsPolicy() && newPassword === confirmPassword;
              ...
              {newPassword && (
                <div className="mt-2">
                  ...
                  <div className="h-1.5 w-full bg-brand-surface-low rounded-full overflow-hidden flex gap-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className={`h-full w-1/5 ${strength.score >= i ? strength.color : 'bg-transparent'}`} />
                    ))}
                  </div>
                </div>
              )}
