...
import { Pin, FileText, Pencil, ImageIcon } from 'lucide-react';
...
function getPreview(note: Note): string {
  if (note.content_type === 'richtext' && note.rich_content) {
    ...
    const content = note.rich_content as { content?: { content?: { text?: string }[] }[] };
    const texts: string[] = [];
    const walk = (nodes: { type?: string; text?: string; content?: unknown[] }[]) => {
      for (const n of nodes) {
        if (n.text) texts.push(n.text);
        if (n.content) walk(n.content as { type?: string; text?: string; content?: unknown[] }[]);
      }
    };
    if (content.content) walk(content.content as { type?: string; text?: string; content?: unknown[] }[]);
...
export const NoteCard: React.FC<NoteCardProps> = ({ note, isActive, onClick, onTogglePin }) => {
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 justify-between">
            ...
            <button
              onClick={onTogglePin}
              className={`p-1 -mr-1 rounded-md transition-all shrink-0 ${
                note.is_pinned
